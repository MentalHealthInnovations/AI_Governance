...
- **`pii-content-sniff.sh`** is the content-level fallback for misnamed files. It reads the first 64 KiB of the target file and scans for emails, UK postcodes, UK phone numbers, UK National Insurance numbers, IBANs, dates of birth, and grouped 16-digit card-shaped sequences. It denies the Read when at least 3 distinct categories appear or any single high-confidence pattern hits 10 or more times. Binaries (NUL byte in the first KiB) are skipped, because the path-policy hook owns those by name. Patterns and thresholds are tunable in-file.
